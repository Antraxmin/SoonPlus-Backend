...
export class DormitoryNoticeService {
  async getDormitoryNotices(): Promise<any[]> {
    try {
      ...
      $('#contents > div > div.jwxe_root.jwxe_board > table > tbody > tr').each(
        (index, element) => {
          if (index < 20) {
            const $titleLink = $(element).find('td.title > a');
            ...
            const author = $($tds[2]).text().trim(); // 작성자

            const date = $($tds[3]).text().trim(); // 날짜 텍스트 추출
